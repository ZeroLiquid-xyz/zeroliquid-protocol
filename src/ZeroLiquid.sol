...
/// @title  ZeroLiquid
/// @author ZeroLiquid
contract ZeroLiquid is IZeroLiquid, Initializable, Multicall, Mutex {
    ...
    /// @inheritdoc IZeroLiquidImmutables
    ...
    /// @inheritdoc IZeroLiquidState
    address public override steamer;
        ...
        lock
    {
        ...
        _repayLimiters[underlyingToken] = Limiters.createLinearGrowthLimiter(
            config.repayLimitMaximum, config.repayLimitBlocks, config.repayLimitMinimum
        );

        _liquidationLimiters[underlyingToken] = Limiters.createLinearGrowthLimiter(
            config.liquidationLimitMaximum, config.liquidationLimitBlocks, config.liquidationLimitMinimum
        );
    ...
    /// @inheritdoc IZeroLiquidAdminActions
    function setSteamer(address value) external override {
        _onlyAdmin();
        _checkArgument(value != address(0));
        steamer = value;
        emit SteamerUpdated(value);
    }
        ...
        returns (uint256)
    {
        ...
        //
        // It is implied that this value is greater than zero because `debt` is greater than zero so a noop is not
        // possible
        ...
        // Transfer the repaid tokens to the steamer.
        TokenUtils.safeTransferFrom(underlyingToken, msg.sender, steamer, actualAmount);

        // Inform the steamer that it has received tokens.
        IERC20TokenReceiver(steamer).onERC20Received(underlyingToken, actualAmount);
        ...
        returns (uint256)
    {
        ...
        //
        // It is implied that this value is greater than zero because `debt` is greater than zero. Casting the debt to
        // an
        // unsigned integer is also safe for this reason.
        uint256 maximumShares = convertUnderlyingTokensToShares(
            yieldToken, normalizeDebtTokensToUnderlying(underlyingToken, uint256(unrealizedDebt))
        );
        ...
        // Transfer the liquidated tokens to the steamer.
        TokenUtils.safeTransfer(underlyingToken, steamer, amountUnderlyingTokens);
    ...
    function harvest(address yieldToken, uint256 minimumAmountOut) external override lock {
        ...
        // Transfer the tokens to the fee receiver and steamer.
        TokenUtils.safeTransfer(underlyingToken, protocolFeeReceiver, feeAmount);
        TokenUtils.safeTransfer(underlyingToken, steamer, distributeAmount);

        // Inform the steamer that it has received tokens.
        IERC20TokenReceiver(steamer).onERC20Received(underlyingToken, distributeAmount);
    ...
    function _wrap(address yieldToken, uint256 amount, uint256 minimumAmountOut) internal returns (uint256) {
        YieldTokenParams storage yieldTokenParams = _yieldTokens[yieldToken];
    ...
    ///
    /// @dev The virtual active balance is the active balance minus any harvestable tokens which have yet to be
    /// realized.
    ///
