...
contract MockstETH is IERC20 {
    ...
    function _transferShares(address _sender, address _recipient, uint256 _sharesAmount) internal {
        ...
        uint256 currentSenderShares = _userShares[_sender];
        require(_sharesAmount <= currentSenderShares, "TRANSFER_AMOUNT_EXCEEDS_BALANCE");

        _userShares[_sender] = currentSenderShares.sub(_sharesAmount);
        _userShares[_recipient] = _userShares[_recipient].add(_sharesAmount);
    }
    ...
    function _mintShares(address _recipient, uint256 _sharesAmount) internal returns (uint256) {
        ...
        uint256 newTotalShares = _getTotalShares().add(_sharesAmount);
    ...
    function _burnShares(address _account, uint256 _sharesAmount) internal returns (uint256) {
        ...
        uint256 userShares = _userShares[_account];
        require(_sharesAmount <= userShares, "BURN_AMOUNT_EXCEEDS_BALANCE");

        uint256 preRebaseTokenAmount = getPooledEthByShares(_sharesAmount);

        uint256 newTotalShares = _getTotalShares().sub(_sharesAmount);
        _totalShares = newTotalShares;

        _userShares[_account] = userShares.sub(_sharesAmount);
    ...
    function submit(address _referral) external payable returns (uint256) {
        ...
        emit Transfer(address(0), msg.sender, getPooledEthByShares(sharesAmount));
